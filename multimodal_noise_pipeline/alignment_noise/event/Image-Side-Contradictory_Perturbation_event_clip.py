...
OUTPUT_ROOT = "rumie_datasets/m2e2/alignment_noise/Image-Side-Contradictory_Perturbation_clip"
...
# =========================================================
TRIGGER_PROMPT = """
You are generating hard negatives for a multimodal EVENT TRIGGER EXTRACTION dataset.

Given:
- A sentence (context)
- An event label (event type, possibly with a hint after comma)

Generate ONE confusing event trigger word or short trigger phrase that:
1) Is plausible in the given context and relevant to the event label.
2) Is semantically close to other likely triggers (easily confusable).
3) Increases ambiguity or mild inconsistency for trigger extraction, but still sounds natural.
4) Must be SHORT (no more than 6 words).
5) Output ONLY the trigger word/phrase. No explanations.
...
# 5. IMAGE EDIT PROMPT
# =========================================================
def build_image_edit_prompt_from_trigger(trigger_phrase: str, event_label: str) -> str:
    return f"""
    ...
    def process_chunk(chunk):
        for item in tqdm(chunk):
            ...
            try:
                ...
                # mask 选区：这里你可继续用 confuse_trigger（作为文本）做 CLIP 选区
                generate_dynamic_mask(image_path=norm_image_path, text=confuse_trigger, mask_path=mask_path)
                prompt = build_image_edit_prompt_from_trigger(trigger_phrase=confuse_trigger, event_label=label)
