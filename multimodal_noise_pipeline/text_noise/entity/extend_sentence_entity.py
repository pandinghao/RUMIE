extend_sentence_entity_prompt = '''The above is a Named Entity Recognition data entry, where the "sentence" contains the sentence information, and the "entities" contains the entity label information. \
...
Please output in the following format, without any additional content: {"sentence": "", "entities": []}, where "sentence" should be the expanded sentence and follow the original token list format, and "entities" should contain the original entity label information.
'''
import openai
import json
                    ...
                    messages=[
                        {
                            "role": "user",
                            "content": [
                                {"type": "text", "text": example_data + extend_sentence_entity_prompt},
                            ]
                        }
                    ],
                    temperature=0.5,
...
if __name__ == "__main__":
    ...
    output_file_path = "rumie_datasets/twitter17/text_noise/extend_sentence/test.jsonl"
    ...
    if fix_flag:
        fix_data = convert_twitter_merge_jsonl(input_path= "rumie_datasets/twitter15/text_noise/extend_sentence/test.jsonl")
        new_data = []
        for index,line in enumerate(fix_data):
            if line == {}:
                data[index]['line_num'] = index + 1
                new_data.append(data[index])
        output_file_path = "rumie_datasets/twitter15/text_noise/extend_sentence/test_fix.jsonl"
        data = new_data
    data = data
